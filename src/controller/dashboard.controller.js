const { adminDashboard, headOfInstitutionDashboard, getEmployeeDashboard } = require("../services/dashboard.service");
...
const getEmployeeDashboardController = async (req, res, next) => {
  try {
    const data = await getEmployeeDashboard(req);
    return success(res, data, "Dashboard berhasil diambil!", 200);
  } catch (error) {
    next(error);
  }
